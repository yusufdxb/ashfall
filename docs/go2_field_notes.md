...
## 9. What this changes for Ashfall

Ashfall's phase-I null result was uninformative because the treatment was never
delivered, not because the hypothesis was disconfirmed. Any hardware arm has to prove
delivery before it proves anything else, and section 1 is exactly how that proof fails
silently: a policy reading a dead topic produces a clean-looking run with no treatment in
it.

Concretely, before any hardware episode counts as an arm:

- `ros2 topic hz` on every input the policy actually reads, not `ros2 topic list`.
- Per-episode telemetry recorded on the payload with a **set clock** (section 6), or the
  episode timestamps cannot be aligned with anything else.
- The gait envelope (section 4) bounds the achievable command range. An arm that requests
  velocities the stock `mcf` policy cannot track cleanly is measuring the policy's
  ceiling, not the failure mode under study.

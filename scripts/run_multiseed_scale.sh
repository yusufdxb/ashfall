#!/bin/bash
# Run the 2026-05-07 multi-seed scaling pass (8 cells, ~2 hours total).
# 2 ff values (0.0, 0.5) x 4 seeds (99, 314, 1729, 2718).
# Each cell: 200-iter fine-tune on slippery + eval on rough+slippery.
# Combines with the n=3 pilot for an n=7 verdict.
set -uo pipefail
...
# All 8 cells share the same generation timestamp from prepare_run.
# Auto-detect by globbing one cell's stamp dir.
SAMPLE_CELL_DIR=$(ls -d "$ASHFALL_ROOT/results/multiseed_scale_2026-05-07_failure_fraction=0p0_seed=99"/*/ 2>/dev/null | head -1)
if [ -z "$SAMPLE_CELL_DIR" ]; then
    echo "ERROR: no generated cells under $ASHFALL_ROOT/results/multiseed_scale_2026-05-07_*" >&2
    echo "Run scripts/run_ablation.sh configs/experiments/multiseed_scale_2026-05-07.yaml first." >&2
    exit 1
fi
STAMP=$(basename "${SAMPLE_CELL_DIR%/}")
echo "[multiseed-scale] using stamp $STAMP"

MANIFEST="$LOGS_DIR/multiseed_scale_manifest.txt"
echo "# Ashfall multi-seed scale manifest" > "$MANIFEST"
...
for cell in "${CELLS[@]}"; do
    CELL_NAME="multiseed_scale_2026-05-07_failure_fraction=${cell}"
    CELL_DIR="$ASHFALL_ROOT/results/${CELL_NAME}/${STAMP}"
    LOG="$LOGS_DIR/multiseed_scale_${cell//=/_}.log"
    echo "" | tee -a "$MANIFEST"
